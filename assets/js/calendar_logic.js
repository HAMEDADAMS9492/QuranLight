 ...
 */
const islamicEvents = [
    { day: 1, month: 'Ramadan', title: 'Début du Ramadan', desc: 'Mois de jeûne, de prière et de réflexion.' },
    { day: 27, month: 'Ramadan', title: 'Laylat al-Qadr', desc: 'La nuit la plus sainte de l\'année.' },
    { day: 1, month: 'Shawwal', title: 'Aïd al-Fitr', desc: 'Célébration de la fin du Ramadan.' },
    { day: 10, month: 'Dhuʻ l-Hijjah', title: 'Aïd al-Adha', desc: 'Fête du sacrifice et fin du Hajj.' },
    { day: 1, month: 'Muharram', title: 'Nouvel An Hégirien', desc: 'Premier jour de l\'année islamique.' },
    { day: 10, month: 'Muharram', title: 'Achoura', desc: 'Jour de jeûne et de commémoration.' },
    { day: 12, month: 'Rabiʻ al-awwal', title: 'Mawlid al-Nabi', desc: 'Naissance du Prophète Muhammad (saws).' }
];
...
function updateHeaderDate() {
    gregorianDateDisplay.textContent = `Aujourd'hui : ${gregorianFormatterFull.format(today)}`;
...
function renderCalendar() {
    ...
    // Titre : Double affichage (ex: Décembre 2025 | Rajab 1447)
    ...
    calendarMonthYear.innerHTML = `
        ...
        <span style="color:var(--gold); opacity:0.5; margin:0 10px;">|</span> 
        <span style="color:var(--gold-bright)">${hijriMonthStr}</span>
    `;
    ...
    eventsList.innerHTML = ''; // Réinitialiser la liste des événements
    let foundAnyEvent = false;

    // 1. Padding (Jours vides avant le 1er du mois)
    ...
    // 2. Création des jours
    for (let d = 1; d <= daysInMonth; d++) {
        const date = new Date(year, month, d);
        const dayElement = document.createElement('div');
        dayElement.classList.add('day');
        
        // Extraction du jour et du mois Hégirien pour ce jour précis
        ...
        if (event) {
            dayElement.style.border = "1px solid var(--gold)";
    ...
    if (!foundAnyEvent) {
        eventsList.innerHTML = `<p style="text-align:center; opacity:0.5; padding:20px;">Aucun événement majeur pour ce mois.</p>`;
    }
}
